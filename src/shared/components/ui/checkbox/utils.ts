import { ColorTypes, colorVariables, scaleWidth } from '@/app/theme';
import { FlexStyle, StyleProp, StyleSheet, TextStyle, ViewStyle } from 'react-native';

export type CheckboxPosition = 'left' | 'right' | 'top' | 'bottom';

export type CheckboxContainedMode = 'contained' | 'separated' | 'card' | 'plain' | 'list' | 'grid' | 'bordered';
  ...
  const colorStyle: ViewStyle = {
    borderColor: disabled
      ...
      : isChecked
        ...
        : (tintColor ?? colors.checkbox.borderBox),
    backgroundColor: disabled ? 'transparent' : isChecked ? (tintColor ?? colors.checkbox.label) : 'transparent',
  };
...
const createSpacingStyle = (checkboxPosition: CheckboxPosition, spacing: number): ViewStyle => {
...
const createLabelTextStyle = ({
  ...
  const defaultLabelStyle = StyleSheet.flatten([baseLabelStyle, spacingForItem, labelStyle, disabledStyles]);
...
export const itemModeStyles: Record<CheckboxContainedMode, StyleProp<ViewStyle>> = {
  contained: {},
  separated: {},
  card: {},
...
export { createCheckboxStyle, createIndeterminateStyle, createLabelTextStyle, createSpacingStyle };
