import { scaleHeight, scaleWidth } from '@/app/theme';
import { getThemeColors, isImageSource, isReactElement } from '@/shared/utils';
import { AppearanceMode } from '@/types/theme';
import { normalize } from '@rneui/themed';
import React, { forwardRef, memo, ReactNode, useCallback, useLayoutEffect, useMemo, useRef, useState } from 'react';
import {
  Animated,
  Image,
  ImageURISource,
...
import { CHILDREN_CONTAINER_MARGIN_TOP, getSizeStyles, ToggleLabelPosition, ToggleSize } from './utils';
...
export interface ToggleProps extends ViewProps {
  ...
  size?: ToggleSize;
  /**
   * The visual theme of the toggle switch. Defaults to 'system' (light/dark mode aware).
   */
...
const Toggle = forwardRef<View, ToggleProps>(
  (
    ...
    const thumbPosition = useRef(
      new Animated.Value(currentValue ? sizeStyles.width - sizeStyles.thumbSize : 0)
    ).current;
    ...
    }, [currentValue, disabled, sizeStyles, style]);
    ...
    const renderIcon = useCallback((): React.JSX.Element | null => {
      ...
      if (isImageSource(Icon)) {
        return (
          <Image testID="image-icon" source={Icon} style={styles.icon} resizeMode="contain" tintColor={iconFillColor} />
        );
      }
      ...
      if (label && labelPosition !== 'hidden') {
        return (
          <View style={styles.toggleAndIconContainer}>
            {labelPosition === 'left' && iconPosition === 'left' && Icon && renderIcon()}
            <Text style={[{ color: colors.toggleSwitch.toggleLabel }, labelStyle]}>{label}</Text>
            {labelPosition === 'right' && iconPosition === 'left' && Icon && renderIcon()}
          </View>
        );
      }
      return null;
    ...
    return (
      <View style={styles.container} {...props}>
        <View style={styles.buttonContainer}>
          ...
          <View style={styles.toggleAndIconContainer}>
            ...
            <TouchableWithoutFeedback
              ...
              testID={props.testID}
            >
              <View ref={ref} style={trackStyle} role={props.role}>
        ...
        {(controlledValue || isShowChildren) && children && (
          <Animated.View style={[styles.childrenContainer, { opacity: childrenOpacity }]}>{children}</Animated.View>
        )}
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  childrenContainer: {
    marginTop: normalize(CHILDREN_CONTAINER_MARGIN_TOP),
    opacity: 0,
  },
});
