...
import { getRadioVariantStyles, RadioButtonVariantStyles } from './utils';

/**
 * Props for the {@link RadioButton} component.
 */
export interface RadioButtonProps extends Omit<PressableProps, 'style'> {
  /**
  ...
  /**
   * Specifies the color theme to use for the radio button. Defaults to 'system', which respects
   * the user's device appearance settings. Other options include 'light' and 'dark'.
   */
...
const RadioButton = forwardRef<View, RadioButtonProps>(
  (
    ...
    const variantStyleSet = useMemo(() => getRadioVariantStyles(variantStyles, colors), [variantStyles, colors]);
    ...
    const animatedInnerCircleStyle = useMemo<Animated.AnimatedProps<ViewStyle>>(
      () => ({
        ...
        backgroundColor: selected ? (tintColor ? tintColor : colors.radioButton.innerCircleSelected) : 'transparent',
        transform: [{ scale: innerScale }],
        ...((StyleSheet.flatten(innerCircleStyle) || {}) as ViewStyle),
      }),
      [innerSize, selected, tintColor, colors.radioButton.innerCircleSelected, innerScale, innerCircleStyle]
    );

    const labelTextStyle = useMemo<TextStyle>(() => {
      const selectedStyle = selected
        ? StyleSheet.flatten([{ color: colors.radioButton.label }, selectedLabelStyle])
        : {
            color: colors.radioButton.label,
          };
      const disabledStyles = disabled ? (disabledStyle ?? { color: colors.radioButton.disabledText }) : {};

      return StyleSheet.flatten([styles.label, selectedStyle, labelStyle, disabledStyles]);
    }, [
      selected,
      ...
      >
        {(state) => (
          <View testID="radio-button-container">
            <View style={styles.rowBetween}>
              ...
              <View style={styles.row}>
                <View style={outerCircle} testID="outer-circle">
                  <Animated.View style={animatedInnerCircleStyle} testID="inner-circle" />
                </View>
            ...
            {/* Children */}
            {isShowChildren && (
              <View>{typeof props.children === 'function' ? props.children(state) : props.children}</View>
            )}
          </View>
        )}
      </Pressable>
